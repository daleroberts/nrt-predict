...
DEFAULT_CONFIG_YAML_FILEPATH="nrtpredict/config/nrt-default-config.yaml"
...
# TODO: We should aim to separate the CLI entry point from the module details into separate files. Some of the content of this method appears to be specific to the module details.
def run(
    url=None,
    urlprefix=None,
    obstmp=None,
...
def cli_entry(url=None, **kwargs):
    ...
    parser.add_argument("-config", default=DEFAULT_CONFIG_YAML_FILEPATH, metavar=('yamlfile'))
    ...
    try:
        ...
        warning(f"\nLoading default configuration settings from '{DEFAULT_CONFIG_YAML_FILEPATH}' ")
        
        #data_dir = os.path.join(os.path.dirname(__file__),'config')
        print(f"\n'{__name__}'")
        file_stream = pkg_resources.resource_stream('nrtpredict', 'config/nrt-default-config.yaml')
        # print(f"\n'{data_path}'")
        # file_stream = os.path.join(data_dir, DEFAULT_CONFIG_YAML_FILE)

        try:
            fargs = yaml.safe_load(file_stream)
            if fargs:
                args = {**args, **fargs}
        except Exception as e:
